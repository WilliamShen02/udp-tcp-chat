...
int main(int argc, char const *argv[]) {
    if (argc != 10) {
        printf("​Invalid command.\nValid command: ./chatclient_tcp -join -host <hostname> -port <port> -username <username> -passcode <passcode>\n");
        return -1;
    }
    ...
    send(client_socket, passcode, sizeof(passcode), 0);
    ...
    while (1) {
        ...
        if (FD_ISSET(client_socket, &file_descriptors)) {
            ...
            int status = recv(client_socket, &response, sizeof(response), 0);
            printf("%s", response);
            if (status == 0) break;
        }
